...
double XenSolver::timeDiff(const std::chrono::time_point<std::chrono::system_clock> &t0, const std::chrono::time_point<std::chrono::system_clock> &t1) {
	return std::chrono::duration<double>(t1 - t0).count();
}

double XenSolver::since(const std::chrono::time_point<std::chrono::system_clock> &t0) {
...
int XenSolver::getResponse(string guess, string solution) {
	int g = 0;
	for (int i = 0; i < 5; ++i) {
		if (i > 0)
			g *= 3;
...
vector<int> XenSolver::splitIntoPartsCount(vector<short> const &solutions, vector<short> const & solutionClue) {
...
// given a list of possible solutions and the solutionClue data for a specific guess word,
// return a separate list for each of the 243 possible responses.
vector<vector<short>> XenSolver::splitIntoParts(vector<short> const &solutions, vector<short> const & solutionClue) {
...
int XenSolver::splitIntoPartsMax(vector<short> const &solutions, vector<short> const & solutionClue) {
	vector<int> count = splitIntoPartsCount(solutions, solutionClue);
	return *max_element(count.begin(), count.end());
}

// show the response in a printable form, using the Wordle Unicode characters
string XenSolver::responseToString(int r) {
...
array<int, 5> XenSolver::collectDead(string w, int response, array<int, 5> const & deadLetters) {
...
XenSolver::Path XenSolver::exploreGuess(vector<short> const & solution, int g, vector<vector<short>> const & solutionClue, vector<string> const & guess,
...
void XenSolver::showPath(ofstream & treefile, string const & prefix, Path const & path, vector<string> const & guess, vector<vector<short>> const & solutionClue, int depth) {
	if (path.guess < 0)
		return;
	if (path.nSolution <= 0)
		return;
...
void XenSolver::pushSolutions(vector<int> & sols, Path const & path) {
	if (path.choices.empty()) {
		for (int i = 0; i < min(5, path.nSolution); ++i) {
...
void XenSolver::showSolutions(ostream & tableFile, Path const & path, vector<string> const & guess) {
	vector<int> sols;
...
void XenSolver::showTable(ostream & tableFile, Path const & path, vector<string> const & guess) {
...
void XenSolver::solve(vector<string> &guess, vector<string> &solutions) {
...
//    int nWords = int(words.size()); // 12972
	...
	for (int it = 0; it < nThread; ++it) {
		ethreads.emplace_back([this, it, nThread, nSolution, te0, &nTogo, &nDone, &next, &favs, &guess, &solution, &partitions, &solutionClue, &score, &favMax, &favEV, &summstreams, &wordstreams]() {
